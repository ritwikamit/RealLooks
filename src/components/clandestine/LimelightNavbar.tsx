...
export const LimelightNavbar: React.FC<LimelightNavbarProps> = ({
  ...
  return (
    <header 
      className={`sticky top-0 z-50 w-full transition-all duration-300 border-b border-[#D6A838]/30 ${
        isScrolled
          ? 'py-2 sm:py-2.5 shadow-[0_10px_30px_rgba(76,91,46,0.08)]'
          : 'py-2.5 sm:py-3.5 shadow-[0_4px_20px_rgba(76,91,46,0.05)]'
      }`}
      style={{ 
        background: 'linear-gradient(135deg, rgba(255, 255, 255, 0.96) 0%, rgba(250, 249, 245, 0.94) 45%, rgba(246, 241, 230, 0.96) 100%)',
    ...
    >
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        {/* 1. Left: Official Brand Logo directly in header with high clarity on luxury gradient */}
        <button
          onClick={() => handleItemClick(0, navItems[0])}
          className="flex items-center cursor-pointer focus:outline-none transition-all hover:scale-[1.03] flex-shrink-0"
          aria-label="Real Looks Home"
        >
          <img
            src="/images/logo.png"
            alt="Real Looks Unisex Salon"
            className="h-11 sm:h-12 md:h-14 w-auto object-contain select-none"
            loading="eager"
          />
        </button>
        ...
        >
          ...
          <div
            ...
            style={{
              boxShadow: '0 3px 14px 1px rgba(214,168,56,0.65), 0 0 8px #F5D77F',
          ...
          >
            ...
            <div
              className="absolute left-[-20%] top-[3.5px] w-[140%] h-[50px] pointer-events-none opacity-30"
              style={{
                clipPath: 'polygon(15% 100%, 30% 0%, 70% 0%, 85% 100%)',
                background: 'linear-gradient(to bottom, rgba(214,168,56,0.45), rgba(214,168,56,0.08) 55%, transparent)',
                filter: 'drop-shadow(0 0 8px rgba(214,168,56,0.4))',
              }}
            />
          </div>
              ...
              return (
                <button
                  ...
                  className={`relative px-3 sm:px-3.5 lg:px-4 py-2 text-[11px] lg:text-xs font-bold tracking-wider uppercase transition-colors duration-150 cursor-pointer select-none ${
                    isSelected
                      ? 'text-[#8E680E] font-black drop-shadow-[0_1px_2px_rgba(214,168,56,0.25)] scale-[1.03]'
                      : 'text-[#2F3B1A]/85 hover:text-[#8E680E] active:scale-95'
                  }`}
                >
                  {item.label}
                </button>
        ...
        <div className="hidden sm:flex items-center gap-3">
          ...
          <div className="indicator">
            ...
            <button
              ...
              className="btn btn-sm btn-ghost bg-[#2F3B1A]/8 hover:bg-[#2F3B1A]/15 text-[#2F3B1A] border border-[#2F3B1A]/15 rounded-xl text-xs font-bold gap-2 px-3.5 transition-all cursor-pointer shadow-2xs active:scale-95"
            >
        ...
        <div className="flex md:hidden items-center gap-2">
          <div className="indicator">
            ...
            <button
              onClick={onOpenBookings}
              className="p-2 rounded-xl bg-[#2F3B1A]/8 text-[#2F3B1A] border border-[#D6A838]/40 cursor-pointer"
          ...
          {navItems.map((item, idx) => (
            <button
              ...
              className={`w-full text-left px-4 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-colors ${
                activeIndex === idx
                  ? 'bg-[#D6A838]/25 text-[#5A4004] font-black'
                  : 'text-[#2F3B1A]/85 hover:bg-[#2F3B1A]/8'
              }`}
            >
              {item.label}
            </button>
          ))}
          <div className="pt-2 border-t border-[#2F3B1A]/10">
            <button
              onClick={() => {
                setMobileMenuOpen(false);
                onBookClick();
